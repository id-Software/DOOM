...
typedef struct RenderTexture2D {
    ...
    boolean depthTexture;      // Track if depth attachment is a texture or renderbuffer
} RenderTexture2D;
...
typedef struct RayHitInfo {
    boolean hit;               // Did the ray hit something?
...
RLAPI boolean WindowShouldClose(void);                               // Check if KEY_ESCAPE pressed or Close icon pressed
RLAPI void CloseWindow(void);                                     // Close window and unload OpenGL context
RLAPI boolean IsWindowReady(void);                                   // Check if window has been initialized successfully
RLAPI boolean IsWindowMinimized(void);                               // Check if window has been minimized (or lost focus)
RLAPI boolean IsWindowResized(void);                                 // Check if window has been resized
RLAPI boolean IsWindowHidden(void);                                  // Check if window is currently hidden
RLAPI boolean IsWindowFullscreen(void);                              // Check if window is currently fullscreen
...
RLAPI boolean IsCursorHidden(void);                                  // Check if cursor is not visible
...
RLAPI boolean FileExists(const char *fileName);                      // Check if file exists
RLAPI boolean IsFileExtension(const char *fileName, const char *ext);// Check file extension
RLAPI boolean DirectoryExists(const char *dirPath);                  // Check if a directory path exists
...
RLAPI boolean ChangeDirectory(const char *dir);                      // Change working directory, returns true if success
RLAPI boolean IsFileDropped(void);                                   // Check if a file has been dropped into window
...
RLAPI boolean IsKeyPressed(int key);                             // Detect if a key has been pressed once
RLAPI boolean IsKeyDown(int key);                                // Detect if a key is being pressed
RLAPI boolean IsKeyReleased(int key);                            // Detect if a key has been released once
RLAPI boolean IsKeyUp(int key);                                  // Detect if a key is NOT being pressed
RLAPI void SetExitKey(int key);                               // Set a custom key to exit program (default is ESC)
RLAPI int GetKeyPressed(void);                                // Get key pressed, call it multiple times for chars queued

// Input-related functions: gamepads
RLAPI boolean IsGamepadAvailable(int gamepad);                   // Detect if a gamepad is available
RLAPI boolean IsGamepadName(int gamepad, const char *name);      // Check gamepad name (if available)
RLAPI const char *GetGamepadName(int gamepad);                // Return gamepad internal name id
RLAPI boolean IsGamepadButtonPressed(int gamepad, int button);   // Detect if a gamepad button has been pressed once
RLAPI boolean IsGamepadButtonDown(int gamepad, int button);      // Detect if a gamepad button is being pressed
RLAPI boolean IsGamepadButtonReleased(int gamepad, int button);  // Detect if a gamepad button has been released once
RLAPI boolean IsGamepadButtonUp(int gamepad, int button);        // Detect if a gamepad button is NOT being pressed
RLAPI int GetGamepadButtonPressed(void);                      // Get the last gamepad button pressed
RLAPI int GetGamepadAxisCount(int gamepad);                   // Return gamepad axis count for a gamepad
RLAPI float GetGamepadAxisMovement(int gamepad, int axis);    // Return axis movement value for a gamepad axis

// Input-related functions: mouse
RLAPI boolean IsMouseButtonPressed(int button);                  // Detect if a mouse button has been pressed once
RLAPI boolean IsMouseButtonDown(int button);                     // Detect if a mouse button is being pressed
RLAPI boolean IsMouseButtonReleased(int button);                 // Detect if a mouse button has been released once
RLAPI boolean IsMouseButtonUp(int button);                       // Detect if a mouse button is NOT being pressed
...
RLAPI boolean IsGestureDetected(int gesture);                    // Check if a gesture have been detected
...
RLAPI boolean CheckCollisionRecs(Rectangle rec1, Rectangle rec2);                                           // Check collision between two rectangles
RLAPI boolean CheckCollisionCircles(Vector2 center1, float radius1, Vector2 center2, float radius2);        // Check collision between two circles
RLAPI boolean CheckCollisionCircleRec(Vector2 center, float radius, Rectangle rec);                         // Check collision between circle and rectangle
RLAPI Rectangle GetCollisionRec(Rectangle rec1, Rectangle rec2);                                         // Get collision rectangle for two rectangles collision
RLAPI boolean CheckCollisionPointRec(Vector2 point, Rectangle rec);                                         // Check if point is inside rectangle
RLAPI boolean CheckCollisionPointCircle(Vector2 point, Vector2 center, float radius);                       // Check if point is inside circle
RLAPI boolean CheckCollisionPointTriangle(Vector2 point, Vector2 p1, Vector2 p2, Vector2 p3);               // Check if point is inside a triangle
...
RLAPI void DrawTextRec(Font font, const char *text, Rectangle rec, float fontSize, float spacing, boolean wordWrap, Color tint);   // Draw text using font inside rectangle limits
RLAPI void DrawTextRecEx(Font font, const char *text, Rectangle rec, float fontSize, float spacing, boolean wordWrap, Color tint,
...
RLAPI boolean TextIsEqual(const char *text1, const char *text2);                               // Check if two text string are equal
...
RLAPI boolean IsModelAnimationValid(Model model, ModelAnimation anim);                                     // Check model animation skeleton match
...
RLAPI boolean CheckCollisionSpheres(Vector3 centerA, float radiusA, Vector3 centerB, float radiusB);       // Detect collision between two spheres
RLAPI boolean CheckCollisionBoxes(BoundingBox box1, BoundingBox box2);                                     // Detect collision between two bounding boxes
RLAPI boolean CheckCollisionBoxSphere(BoundingBox box, Vector3 center, float radius);                      // Detect collision between box and sphere
RLAPI boolean CheckCollisionRaySphere(Ray ray, Vector3 center, float radius);                              // Detect collision between ray and sphere
RLAPI boolean CheckCollisionRaySphereEx(Ray ray, Vector3 center, float radius, Vector3 *collisionPoint);   // Detect collision between ray and sphere, returns collision point
RLAPI boolean CheckCollisionRayBox(Ray ray, BoundingBox box);                                              // Detect collision between ray and box
...
RLAPI boolean IsVrSimulatorReady(void);                    // Detect if VR simulator is ready
...
RLAPI boolean IsAudioDeviceReady(void);                                  // Check if audio device has been initialized successfully
...
RLAPI boolean IsSoundPlaying(Sound sound);                               // Check if a sound is currently playing
...
RLAPI boolean IsMusicPlaying(Music music);                               // Check if music is playing
...
RLAPI boolean IsAudioStreamProcessed(AudioStream stream);                // Check if any audio stream buffers requires refill
RLAPI void PlayAudioStream(AudioStream stream);                       // Play audio stream
RLAPI void PauseAudioStream(AudioStream stream);                      // Pause audio stream
RLAPI void ResumeAudioStream(AudioStream stream);                     // Resume audio stream
RLAPI boolean IsAudioStreamPlaying(AudioStream stream);                  // Check if audio stream is playing
